...
class Train(object):
    ...
    def build_train_validation_graph(self):
        ...
        loss = self.loss(tf.reshape(logits,[-1]), self.label_placeholder)
        ...
        # Validation loss
        self.vali_loss = self.loss(tf.reshape(vali_logits,[-1]), self.vali_label_placeholder)
        ...
        self.val_op = self.validation_op(validation_step, self.vali_loss)
    ...
    def train(self):
        ...
        for step in xrange(FLAGS.train_steps):
            train_batch_data, train_batch_labels = self.generate_batch(all_data, all_labels,
                                                                        FLAGS.train_batch_size)


            validation_batch_data, validation_batch_labels = self.generate_batch(vali_data,
            ...
            if step % FLAGS.report_freq == 0:
                ...
                _, validation_loss_value = sess.run([self.val_op, self.vali_loss],
                ...
                val_error_list.append(validation_loss_value)


            start_time = time.time()

            _, _, train_loss_value = sess.run([self.train_op, self.train_ema_op,self.full_loss],
                                {self.image_placeholder: train_batch_data,
                                  self.label_placeholder: train_batch_labels,
                                  self.vali_image_placeholder: validation_batch_data,
                                  self.vali_label_placeholder: validation_batch_labels,
            ...
            if step % FLAGS.report_freq == 0:
                ...
                format_str = ('%s: step %d, Training loss = %.4f (%.1f examples/sec; %.3f ' 'sec/batch)')
            ...
            if step % 500 == 0 or (step + 1) == FLAGS.train_steps:
                ...
                df = pd.DataFrame(data={'step':step_list, 'train_loss':train_error_list,
                                'validation_loss': val_error_list})
                df.to_csv(train_dir + FLAGS.version + '_error.csv')

    def loss(self, predicts, labels):
        mse2 = tf.losses.mean_squared_error(labels, predicts)
        return mse2


    def generate_batch(self, vali_data, vali_label, vali_batch_size):
        '''
    ...
    def validation_op(self, validation_step, loss):
        '''
        ...
        val_op = tf.group(validation_step.assign_add(1), ema.apply([loss]),
                          ema2.apply([loss]))
    ...
    def full_validation(self, loss, session, vali_data, vali_labels, batch_data,
                        batch_label):
        '''
        ...
        :return: float, float
        '''
        num_batches = 252 // FLAGS.validation_batch_size
        order = np.random.choice(252, num_batches * FLAGS.validation_batch_size)
        ...
        for step in range(num_batches):
            ...
            feed_dict = {self.image_placeholder: batch_data, self.label_placeholder: batch_label,
                ...
                self.lr_placeholder: FLAGS.init_lr}
            loss_value, top1_error_value = session.run([loss], feed_dict=feed_dict)
